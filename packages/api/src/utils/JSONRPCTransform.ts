...
export class JSONRPCTransform extends Transform {
    ...
    private constructor(options?: TransformOptions) {
        ...
        this.on('pipe', (src) => {
            if (!this.readableEncoding && src instanceof Readable) {
                this.setEncoding(src.readableEncoding as any);
            }
        });
    ...
    public _transform(
        ...
        const headerRe = /^Content-Length: ([0-9]+)((\r?\n(.+))*)\r?\n\r?\n/i;
        ...
        while (true) {
            ...
            if (this._state === 'content-length') {
                ...
                this._curChunk = this._curChunk.slice(
                    Buffer.byteLength(match[0], encoding)
                );
                this._state = 'jsonrpc';
            }
            ...
            if (
                this._state === 'jsonrpc' &&
                this._curChunk.length >= this._curContentLength
            ) {
                this.push(
                    this._reencode(
